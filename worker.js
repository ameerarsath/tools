...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.action === 'showStealthAnswer') {
        showStealthAnswer(message.answer);
    }
});

// Show answer in a separate popup window — invisible to tab/window screen share
function showStealthAnswer(answer) {
    chrome.windows.getCurrent((currentWindow) => {
        const popupTop = Math.round(((currentWindow.height || 800) / 2) - 50 + (currentWindow.top || 0));
        const popupLeft = (currentWindow.left || 0) + 20;
        chrome.windows.create({
            url: `data/answer-popup.html?a=${encodeURIComponent(answer)}`,
            type: 'popup',
            width: 240,
            height: 110,
            left: popupLeft,
...
function handleQueryResponseForIamNeoExamly(response, tabId, isMCQ = false, isHackerRank = false, isMultipleChoice = false, isTyped = false, isTCSiON = false) {
    if (response && typeof response === 'string') {
        // Success case - response is the actual text
        if (isMCQ) {
            ...
            // Robustly strip ALL markdown code fences from AI response
            const cleanedCode = response.trim()
                // Remove opening fence with optional language tag (```python, ```java, ``` etc.)
                .replace(/^```[\w]*\s*\n?/gm, '')
            ...
            // TCS iON: try auto-inject into Ace editor, fallback to clipboard
            if (isTCSiON) {
                // Attempt to inject into Ace editor directly
                chrome.scripting.executeScript({
                    target: { tabId: tabId },
                    func: function(code) {
                        // Try the labeled answer editor first
                        var answerEl = document.querySelector('[aria-labelledby="editor-answer"]');
                        var injected = false;
                        if (answerEl) {
                            try { ace.edit(answerEl).setValue(code); ace.edit(answerEl).clearSelection(); injected = true; } catch(e) {}
                        }
                        if (!injected) {
                            // Fallback: first writable Ace editor
                            document.querySelectorAll('.ace_editor').forEach(function(el) {
                                if (injected) return;
                                try {
                                    var ed = ace.edit(el);
                                    if (!ed.getReadOnly()) { ed.setValue(code); ed.clearSelection(); injected = true; }
                                } catch(e) {}
                            });
                        }
                        return injected;
                    ...
                    if (!injected) {
                        // Editor not found — code is already in clipboard
                        showToast(tabId, 'Code copied — Ctrl+A then paste into editor');
                    }
                }).catch(function() {
                    showToast(tabId, 'Code copied — paste into editor');
                });
                return;
            }
...
async function queryCustomAPI(text, isMCQ, isMultipleChoice, config) {
    const { aiProvider, customEndpoint, apiKey, modelName,
            azureEndpoint, azureDeployment, azureApiVersion } = config;
    ...
    try {
        ...
        switch (aiProvider) {
            ...
            case 'azure': {
                // Azure OpenAI: endpoint + deployment + api-version in URL, api-key header
                const deployment = azureDeployment || modelName || 'gpt-4o';
                ...
                if (!baseEndpoint) {
                    return {
                        ...
                        errorType: 'config',
                        detailedInfo: 'Please enter your Azure OpenAI endpoint URL in the extension settings.'
                    };
                }
                apiUrl = `${baseEndpoint}/openai/deployments/${deployment}/chat/completions?api-version=${apiVersion}`;
                console.log('[Azure] Calling URL:', apiUrl);
                console.log('[Azure] Deployment:', deployment, '| API Version:', apiVersion);
                headers = {
                    'Content-Type': 'application/json',
                    'api-key': apiKey
                };
        ...
        if (!response.ok) {
            ...
            console.error('[queryCustomAPI] API error:', response.status, detail);
            return {
    ...
    if (message.action === "testCustomAPI") {
        (async () => {
            try {
                ...
                console.log('[testCustomAPI] Provider:', config.aiProvider, '| Key set:', !!config.apiKey);
                const testPrompt = "Respond with only the word: OK";
                
                const result = await queryCustomAPI(testPrompt, false, false, config);
                console.log('[testCustomAPI] Result:', result);
                ...
                if (typeof result === 'string') {
                    ...
                    const errMsg = result.detailedInfo || result.error
                        || JSON.stringify(result)
                        || 'No error detail returned';
                    console.error('[testCustomAPI] Failed:', errMsg);
                    sendResponse({
                        success: false,
                        error: errMsg
                    });
                ...
                console.error('[testCustomAPI] Exception:', error);
                sendResponse({
                    success: false,
                    error: error.message || String(error) || 'Exception in test handler'
                });
            }
        })();
...
// =====================================================================
// Screenshot + Vision handler for image-based MCQ questions
// =====================================================================
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === 'captureScreenForMCQ') {
        (async () => {
            const tabId = sender.tab.id;
            const windowId = sender.tab.windowId;
            try {
                // Silent capture — no toast shown (would be visible during screen sharing)
                const dataUrl = await chrome.tabs.captureVisibleTab(windowId, { format: 'jpeg', quality: 75 });
                const base64Image = dataUrl.split(',')[1];

                const config = await getCustomAPIConfig();
                const { aiProvider, apiKey, modelName,
                        azureEndpoint, azureDeployment, azureApiVersion } = config;

                if (!apiKey) {
                    // No toast — log silently to service worker console
                    console.error('[Vision MCQ] No API key configured');
                    sendResponse({ success: false, error: 'No API key' });
                    return;
                }

                const visionPrompt = `Aptitude exam question with a visual figure/pattern matrix.
${request.questionText ? 'Question: ' + request.questionText : ''}
${request.optionsText ? 'Options shown: ' + request.optionsText : ''}
Analyze the visual pattern. Which option number is correct?
Reply with ONLY a single digit (e.g.: 3). No explanation.`;
                ...
                switch (aiProvider) {
                    case 'azure': {
                        ...
                        if (!base) throw new Error('Azure endpoint not configured');
                        apiUrl = `${base}/openai/deployments/${deployment}/chat/completions?api-version=${apiVersion}`;
                        headers = { 'Content-Type': 'application/json', 'api-key': apiKey };
                        requestBody = { messages: [{ role: 'user', content: visionContent }], temperature: 0.1, max_tokens: 10 };
                        break;
                    }
                    case 'openai':
                        apiUrl = 'https://api.openai.com/v1/chat/completions';
                        headers = { 'Content-Type': 'application/json', 'Authorization': `Bearer ${apiKey}` };
                        requestBody = { model: modelName || 'gpt-4o', messages: [{ role: 'user', content: visionContent }], temperature: 0.1, max_tokens: 10 };
                        break;
                    case 'google': {
                        const gModel = modelName || 'gemini-2.5-flash';
                        apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/${gModel}:generateContent?key=${apiKey}`;
                        headers = { 'Content-Type': 'application/json' };
                        requestBody = { contents: [{ parts: [{ inline_data: { mime_type: 'image/jpeg', data: base64Image } }, { text: visionPrompt }] }] };
                        break;
                    }
                    default:
                        throw new Error(`Vision not supported for provider "${aiProvider}". Use Azure, OpenAI, or Google.`);
                }

                console.log('[Vision MCQ] Provider:', aiProvider, '| URL:', apiUrl);
                ...
                const answer = (aiProvider === 'google'
                    ? data.candidates?.[0]?.content?.parts?.[0]?.text
                    : data.choices?.[0]?.message?.content)?.trim();

                if (!answer) throw new Error('Empty response from vision AI');

                console.log('[Vision MCQ] Answer:', answer);
                chrome.tabs.sendMessage(tabId, {
                    action: 'clickMCQOption',
                    response: answer,
                    isTCSiON: true,
                ...
                // Silent failure — log to service worker console only (not visible during screen sharing)
                console.error('[Vision MCQ] Error:', err.message, err);
...
// Helper: builds the standard (non-expert) coding prompt from a request object
function buildStandardCodingPrompt(request) {
    return `You are a student solving a coding problem. Write ONLY the solution code.
${request.programmingLanguage ? 'Language: ' + request.programmingLanguage : ''}
...
${request.testCases ? '\nTest Cases:\n' + request.testCases : ''}
${request.headerSnippet ? '\nHeader (pre-existing, do NOT include):\n' + request.headerSnippet : ''}
${request.footerSnippet ? '\nFooter (pre-existing, do NOT include):\n' + request.footerSnippet : ''}
${request.whitelist ? '\nMUST use these keywords:\n' + request.whitelist : ''}
    ...
    if (request.action === 'extractData') {
        (async () => {
            try {
                ...
                if (request.isCoding) {
                    if (request.isTCSiON) {
                        queryText = `Solve this programming problem. Write ONLY the complete solution code — no explanations, no comments, no markdown.

Requirements:
- Correct algorithm that handles ALL edge cases
- Read input from stdin, write output to stdout
- Handle: empty input, n=0, n=1, large values, overflow, duplicates, boundary conditions
...
${request.outputFormat ? '\nOutput Format:\n' + request.outputFormat : ''}
${request.testCases ? '\nTest Cases:\n' + request.testCases : ''}
${request.headerSnippet ? '\nHeader (already in editor, do NOT repeat):\n' + request.headerSnippet : ''}
${request.footerSnippet ? '\nFooter (already in editor, do NOT repeat):\n' + request.footerSnippet : ''}
${request.whitelist ? '\nMUST use these identifiers:\n' + request.whitelist : ''}

Output ONLY the code:`;
                    } else if (request.isHackerRank) {
                    ...
                    } else if (request.isCodingExpert) {
                        // ── EXPERT MODE: 3-Pass Self-Correction Loop ─────────────────
                        const lang = request.programmingLanguage || 'appropriate language';
                        ...
                        const pass1Prompt = `You are an expert competitive programmer. Analyze this problem thoroughly.

${problemContext}

Constraints given: (extract from problem text above)

Your analysis MUST cover:
1. RESTATE: What does the problem ask in simple terms?
2. CONSTRAINTS: What are the exact input bounds (min/max n, value ranges)?
3. EDGE CASES (list ALL): empty input, single element, all same values, max values, negative numbers, zero, overflow risk, already sorted, reverse sorted, duplicates, etc.
4. ALGORITHM: Choose the correct approach. Justify why.
5. COMPLEXITY: Time and space complexity.
6. TRACE: Step through each sample test case with your algorithm — write actual intermediate values.
7. COMMON BUGS: What mistakes would a typical coder make here?
...
Be thorough. Do NOT write code yet.`;
                        ...
                        if (!analysis || typeof analysis !== 'string') {
                            ...
                            queryText = buildStandardCodingPrompt(request);
                        } else {
                            // ── Pass 2: Generate Initial Code ────────────────────────
                            const pass2Prompt = `You are an expert competitive programmer. You have analyzed this problem:

=== ANALYSIS ===
${analysis}
=== END ANALYSIS ===

Now write the ${lang} solution. Requirements:
- Handle EVERY edge case listed in your analysis
- Read input from stdin, write output to stdout
- Correct algorithm as per your analysis
${request.headerSnippet ? `- Header already exists (do NOT repeat):\n${request.headerSnippet}` : ''}
${request.footerSnippet ? `- Footer already exists (do NOT repeat):\n${request.footerSnippet}` : ''}
${request.whitelist ? `- MUST use these identifiers: ${request.whitelist}` : ''}
...
Respond with ONLY the code. No explanations. No markdown fences.`;
                            ...
                            if (!initialCode || typeof initialCode !== 'string') {
                                ...
                                // ── Pass 3: Self-Correction (Dry Run) ────────────────
                                const pass3Prompt = `You are a strict code reviewer and test engineer.

Here is a ${lang} solution for a competitive programming problem:
...
Edge cases identified earlier:
${analysis.split('\n').filter(l => l.toLowerCase().includes('edge') || l.trim().match(/^[-*•]/) || l.trim().match(/^\d+\./)).slice(0,20).join('\n')}

Your task:
1. TRACE: Manually execute the code step-by-step for EACH sample test case. Write the exact variable values at each step.
2. EDGE TEST: Execute the code for AT LEAST 5 edge cases (empty, single element, max values, all same, boundary). Write actual output vs expected output.
3. BUG CHECK: Is there any integer overflow? Off-by-one? Wrong loop boundary? Incorrect base case? Wrong data type?
4. VERDICT: Does the code pass ALL cases?

If the code has ANY bugs → output the COMPLETE FIXED code only (no explanation, no markdown fences).
If the code is CORRECT → output the EXACT same code unchanged (no explanation, no markdown fences).
...
Output ONLY the final code:`;
                                ...
                                console.log('[Expert Mode] Complete. Code length:', queryText.length);
                            }
                        }

                    } else {
                        // Default: other platforms
                        queryText = `Instructions: You are tasked with solving a programming problem. Respond strictly with the solution code in the required programming language. Ensure the code meets the requirements and passes all test cases including edge cases. Always get the input from the users.` +
                            `Question:\n${request.question}\n\n` +
                            (request.programmingLanguage ? `Language:\n${request.programmingLanguage}\n\n` : '') +
                            (request.inputFormat ? `Input Format:\n${request.inputFormat}\n\n` : '') +
                            (request.outputFormat ? `Output Format:\n${request.outputFormat}\n\n` : '') +
                            (request.testCases ? `Test Cases:\n${request.testCases}` : '') +
                            (request.headerSnippet ? `\n\nHeader Snippet (pre-existing, do NOT include):\n${request.headerSnippet}` : '') +
                            (request.footerSnippet ? `\n\nFooter Snippet (pre-existing, do NOT include):\n${request.footerSnippet}` : '') +
                            (request.whitelist ? `\n\nMUST use these keywords:\n${request.whitelist}` : '');
                   }
                ...
                if (response && typeof response === 'string') {
                    ...
                    handleQueryResponseForIamNeoExamly(response, sender.tab.id, request.isMCQ, request.isHackerRank, request.isMultipleChoice, request.isTyped, request.isTCSiON);
                    sendResponse({
                        success: true,
                        response,
                        status: 'success'
