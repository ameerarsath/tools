document.addEventListener('DOMContentLoaded', function () {
    ...
    const CUSTOM_API_STORAGE_KEYS = ['useCustomAPI', 'aiProvider', 'customEndpoint', 'customAPIKey', 'customModelName', 'azureEndpoint', 'azureDeployment', 'azureApiVersion'];
...
async function fetchAccountInfo() {
    ...
    if (aiProviderSelect) {
        aiProviderSelect.addEventListener('change', function() {
            ...
            const azureFieldsDiv = document.getElementById('azureFieldsDiv');
            if (azureFieldsDiv) {
                if (this.value === 'azure') {
    ...
    // Azure field auto-save listeners
    ['azureEndpoint', 'azureDeployment', 'azureApiVersion'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.addEventListener('input', autoSaveAPIConfig);
    });

    // Direct API test — runs in popup context, no service worker needed (fixes MV3 lifetime issue)
    async function testAPIDirectly(config) {
        ...
        switch (aiProvider) {
            case 'azure': {
                ...
                if (!base) throw new Error('Azure endpoint is empty. Please fill in the Azure Endpoint field.');
                apiUrl = `${base}/openai/deployments/${deployment}/chat/completions?api-version=${apiVersion}`;
                headers = { 'Content-Type': 'application/json', 'api-key': apiKey };
                requestBody = { messages: [{ role: 'user', content: testPrompt }], temperature: 0.1, max_tokens: 10 };
                console.log('[Azure Test] URL:', apiUrl);
                break;
            }
            case 'openai':
                apiUrl = 'https://api.openai.com/v1/chat/completions';
                headers = { 'Content-Type': 'application/json', 'Authorization': `Bearer ${apiKey}` };
                requestBody = { model: modelName || 'gpt-4o-mini', messages: [{ role: 'user', content: testPrompt }], max_tokens: 10 };
                break;
            case 'anthropic':
                apiUrl = 'https://api.anthropic.com/v1/messages';
                headers = { 'Content-Type': 'application/json', 'x-api-key': apiKey, 'anthropic-version': '2023-06-01' };
                requestBody = { model: modelName || 'claude-3-5-sonnet-20241022', max_tokens: 10, messages: [{ role: 'user', content: testPrompt }] };
                break;
            case 'google': {
            ...
            case 'deepseek':
                ...
                headers = { 'Content-Type': 'application/json', 'Authorization': `Bearer ${apiKey}` };
                requestBody = { model: modelName || 'deepseek-chat', messages: [{ role: 'user', content: testPrompt }], max_tokens: 10 };
                break;
            case 'custom':
                if (!customEndpoint) throw new Error('Custom endpoint not configured. Please enter your API endpoint URL.');
    ...
    if (testAPIConfigButton) {
        testAPIConfigButton.addEventListener('click', async function() {
            ...
            try {
                ...
                showError('✗ ' + (error.message || 'Connection failed'), 6000);
            } finally {
